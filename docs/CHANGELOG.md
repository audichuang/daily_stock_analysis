...
- [文档] 澄清调度文档边界：`SCHEDULE_TIME` 仍仅影响本地内置调度（`python main.py --schedule`）；`GitHub Actions` 的触发时点仍由 `00-daily-analysis.yml` 的固定 `cron` 决定（周一到周五 10:00 UTC / 北京18:00），未实现通过 Actions `SCHEDULE_TIME` 动态调整触发时间。
