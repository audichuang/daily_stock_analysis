...
本项目新增独立的 WebUI 界面语言能力（`zh` / `en` / `zh-TW`），用于静态页面文案、导航与通用控件文案。该状态与 `REPORT_LANGUAGE` 解耦，不改写报告语言语义。
...
- 界面切换會同步 `document.documentElement.lang`（`zh-CN`、`zh-TW` 或 `en`）以利於可及性與無障礙語意。
