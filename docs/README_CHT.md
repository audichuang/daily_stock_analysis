...
| 多市場數據聚合 | A股、港股、美股、ETF：行情、K 線、技術指標、資金流、籌碼、新聞、公告和基本面；日股/韓股/台股（`.T` / `.KS` / `.KQ` / `.TW` / `.TWO`）：目前僅支援 YFinance 日線與基礎行情、技術指標，`capital_flow`、`dragon_tiger`、`boards` 與其他高階區塊會依市場邊界降級為 `not_supported`（見 [市場支持邊界](market-support.md)） |
