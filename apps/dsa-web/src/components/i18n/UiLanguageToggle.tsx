...
export const UiLanguageToggle: React.FC<UiLanguageToggleProps> = ({
  ...
  const label = language === 'zh' ? t('language.uiLanguage') : language === 'zh-TW' ? t('language.traditional') : t('language.current');
  ...
  return (
    <div className={cn('relative', isRailVariant ? 'w-full' : '', wrapperClassName)}>
      ...
      >
        ...
        {isRailVariant ? (
          <span className={labelClassName}>{language === 'zh' ? t('language.short.zh') : language === 'zh-TW' ? t('language.short.zhTW') : t('language.short.en')}</span>
        ) : isNavVariant ? (
